...
策略：以欣亞為基準，先配對原價屋（現有引擎），再對每個配對成功的商品搜尋 PCHOME 和 momo 的最佳對應商品
"""
...
def find_best_match(query_name, products, product_index, pos_to_orig,
                    category_compat=None, query_category=""):
    """
    ...
    if best_pos >= 0 and best_score >= 0.65:  # slightly lower threshold for cross-platform
...
def match_all_platforms(sinya_products, coolpc_products, pchome_products, momo_products,
    ...
    2. 對每個配對成功的商品，在 PCHOME 和 momo 中搜尋最佳對應
    3. 回傳包含四平台價格的配對結果
    """
    ...
    # Step 3: 對每個已配對的商品，搜尋 PCHOME 和 momo
    pchome_matched_count = 0
    momo_matched_count = 0

    for m in matched:
        # 使用欣亞品名作為搜尋基準（也可用原價屋品名）
        search_name = m.get("sinya_name", m.get("name", ""))

        # 搜尋 PCHOME
        pchome_prod, pchome_score = find_best_match(
            search_name, pchome_valid, pchome_index, pchome_pos_to_orig
        )
        if pchome_prod:
        ...
        # 搜尋 momo
        momo_prod, momo_score = find_best_match(
            search_name, momo_valid, momo_index, momo_pos_to_orig
        )
        if momo_prod:
