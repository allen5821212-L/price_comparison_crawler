...
import { Bell, BellRing, CheckCheck, Heart, HeartOff, Target } from "lucide-react";
...
export default function FavoritesPage() {
  ...
  return <DashboardLayout><div className="mx-auto w-full max-w-6xl space-y-6">
    ...
    <Card className="overflow-hidden"><div className="border-b border-border p-4"><h2 className="font-semibold">我的收藏</h2></div>{favoritesQuery.isLoading ? <div className="space-y-3 p-4">{Array.from({ length: 3 }, (_, i) => <Skeleton key={i} className="h-16 w-full" />)}</div> : <div className="divide-y divide-border">{favorites.length === 0 ? <p className="p-10 text-center text-sm text-muted-foreground">尚未收藏商品。請由比價列表的愛心按鈕開始追蹤。</p> : favorites.map(item => <div className={!item.active ? "flex items-center gap-4 p-4 opacity-55" : "flex items-center gap-4 p-4"} key={item.id}><Heart className="size-4 shrink-0 text-rose-500" fill={item.active ? "currentColor" : "none"} /><div className="min-w-0 flex-1"><p className="truncate font-medium" title={item.sinyaName}>{item.sinyaName}</p><p className="mt-1 text-xs text-muted-foreground">目前追蹤價格：{formatPrice(item.lastKnownPrice)} · 目標價：{formatPrice(item.targetPrice)}</p></div><Button size="sm" variant="outline" disabled={toggleFavorite.isPending} onClick={() => toggleFavorite.mutate({ id: item.id, active: !item.active })}>{item.active ? <><HeartOff className="mr-2 size-3.5" />停止</> : <><Heart className="mr-2 size-3.5" />恢復</>}</Button></div>)}</div>}</Card>
